document.addEventListener("DOMContentLoaded", () => {
    ...
    // Simulate window.onload if document is already loaded or just run logic
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $(alertContainerId).html(
                        `<div class="alert alert-${response.type === 'success' ? 'success' : (response.type === 'warning' ? 'warning' : 'danger')}" role="alert">${response.message}</div>`
                    );
                    if (response.type === 'warning') {
                        setTimeout(() => {
                            if(formId == "#teacher-form" || formId == "teacher") { // ID selector
                                const prevFlip = document.getElementById("prevFlip");
                                if(prevFlip) prevFlip.click();
                            } else {
                                const sprevFlip = document.getElementById("sprevFlip");
                                if(sprevFlip) sprevFlip.click();
                            }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function handleEmailValidation() {
        ...
        if (isValidEmail && emailErrorSpan) {
    ...
    // Dealing with Arabic name inputs
    if(first_name) {
    ...
    if (window.innerWidth <= 576) {
        ...
        if(nextPage) {
            nextPage.addEventListener("click", function (event) {
                // This duplicates the nextPage logic for mobile?
                // It seems to be handling alert containers differently.
                ...
                const showAlert = (containerId, type, message) => {
                    ...
                    if(el) el.innerHTML = `<div class="alert alert-${type}" role="alert">${message}</div>`;
                };
        ...
        if(nextFlip) {
            nextFlip.addEventListener("click", function (event) {
                // Mobile specific nextFlip logic
                let hasError = false;
