...
if (!$data || !isset($data['searchInput'])) {
...
if ($user['account_type'] === "teacher") {
    $query = "(
                    ...
                    WHERE assigned_assistants.teacher_id = ? AND subject_files.file_name LIKE '$searchInput%'
                )
                UNION
                (
                    SELECT
                    ...
                    WHERE users.id = ? AND subject_files.file_name LIKE '$searchInput%'
                )
    ...
    $query = "SELECT
            ...
            FROM subject_files
            JOIN users ON subject_files.teacher_id = users.id
            WHERE subject_files.teacher_id = ? AND subject_files.file_name LIKE '$searchInput%'
               OR subject_files.teacher_id IN (
                   SELECT teacher_id 
                   FROM assigned_assistants 
                   WHERE assistant_id = ? AND subject_files.file_name LIKE '$searchInput%'
               )
